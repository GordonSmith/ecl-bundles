...
op1 := OUTPUT(TABLE(DataBreach.RawDataset, {BreachType := TypeOfBreach, UNSIGNED INTEGER4 SumIndividualsAffected := SUM(GROUP, IndividualsAffected)}, TypeOfBreach, FEW), NAMED('TypeOfBreach'));
...
op2 := OUTPUT(TABLE(DataBreach.RawDataset, {CoveredEntityType, UNSIGNED INTEGER4 SumIndividualsAffected := SUM(GROUP, IndividualsAffected)}, CoveredEntityType, FEW), NAMED('CoveredEntityType'));
myPieChart := Chart2D.Pie('myPieChart',, 'CoveredEntityType');

//  Aggregate by LocationOfInformation ---
op3 := OUTPUT(TABLE(DataBreach.RawDataset, {LocationOfInformation, UNSIGNED INTEGER4 SumIndividualsAffected := SUM(GROUP, IndividualsAffected)}, LocationOfInformation, FEW), NAMED('LocationOfInformation'));
...
SEQUENTIAL(data_byState, viz_usStates, op1, op2, op3, op4, myColumnChart, myPieChart, myBarChart, myTables);
