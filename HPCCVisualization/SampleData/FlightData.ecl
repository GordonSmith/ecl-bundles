EXPORT SampleData := MODULE
    ...
    EXPORT File := DATASET('~HPCCVisualization::FlightPath', Def, FLAT);

    SHARED IdxFile := DATASET('~HPCCVisualization::FlightPath', {Def, UNSIGNED8 RecPos{VIRTUAL(fileposition)}}, FLAT);
    EXPORT Idx := INDEX(IdxFile, {Orgin_State, Dest_State, RecPos}, '~HPCCVisualization::FlightPathIdx');

    EXPORT createFiles() := FUNCTION
        op := OUTPUT(RawDataset,, '~HPCCVisualization::FlightPath', OVERWRITE);
        RETURN SEQUENTIAL(op, BUILDINDEX(Idx, OVERWRITE));
    END;		
    
    EXPORT createRoxieQuery() := FUNCTION
        varstring origin_state_value := '' : stored('Origin_State');
        varstring dest_state_value := '' : stored('Dest_State');
        
        fetched := FETCH(File, Idx(Origin_State=origin_state_value, Dest_State=dest_state_value), RIGHT.RecPos);
        op := OUTPUT(CHOOSEN(fetched,2000), NAMED('FlightPath'));
        RETURN SEQUENTIAL(op);
    END;
    
    EXPORT main := FUNCTION
        //cf := createFiles();
        //crq := createRoxieQuery();
        //  Manually Publish Query  
        RETURN '';//SEQUENTIAL(cf, crq);
    END;
END;